...
class DeblurTrainer():
    ...
    def train(self):
        ...
        for batch, (blur, sharp, _, connectivity) in enumerate(self.loader_train):
            (cha_src, cha_dst), (spa_src, spa_dst) = connectivity
            blur, sharp, cha_src, cha_dst, spa_src, spa_dst = \
                self.prepare(blur, sharp, cha_src, cha_dst, spa_src, spa_dst)
            timer_data.hold()
            timer_model.tic()
            ...
            input = {
                'x': blur,
                'cha_con': (cha_src, cha_dst),
                'spa_con': (spa_src, spa_dst)
            }
            cleaned = self.model(input)
    ...
    def test(self):
        ...
        for idx_data, (blur, _, filename, connectivity) in enumerate(self.loader_test):
            (cha_src, cha_dst), (spa_src, spa_dst) = connectivity
            print(blur.size())
            lr, cha_src, cha_dst, spa_src, spa_dst = self.prepare(blur, cha_src, cha_dst, spa_src, spa_dst)
            print(lr[0].size())
            input = {
                'x': lr[0],
                'cha_con': (cha_src, cha_dst),
